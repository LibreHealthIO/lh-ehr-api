...
namespace LibreEHR\Core\Lib\Date;
...
// begin on Sunday.
//define('DateCalc_BEGIN_WEEKDAY', 1);

/**
 * DateCalc is a calendar class used to calculate and
...
class DateCalc {
    ...
    static function isValidDate($day, $month, $year)
    {
        ...
        if($month < 1 || $month > 12)
            return false;
        if($day < 1 || $day > 31 || $day > DateCalc::daysInMonth($month,$year))
            return false;
    ...
    {
        $this_year = DateCalc::dateNow("%Y");
        $this_month = DateCalc::dateNow("%m");
        $this_day = DateCalc::dateNow("%d");
    ...
    static function dayOfWeek($day="",$month="",$year="")
    {
        ...
        if(empty($year))
            $year = DateCalc::dateNow("%Y");
        if(empty($month))
            $month = DateCalc::dateNow("%m");
        if(empty($day))
            $day = DateCalc::dateNow("%d");
    ...
    static function weekOfYear($day,$month,$year)
    {
        ...
        $week_number =
                ceil((DateCalc::julianDate($day,$month,$year) + floor(($week_day + 4) % 7)) / 7);
    ...
    static function julianDate($day="",$month="",$year="")
    {
        ...
        if($month > 2 && DateCalc::isLeapYear($year))
            $julian++;
    ...
    static function beginOfNextMonth($day="",$month="",$year="",$format="%Y%m%d")
    {
        ...
        return DateCalc::dateFormat($day,$month,$year,$format);
    ...
    static function endOfNextMonth($day="",$month="",$year="",$format="%Y%m%d")
    {
        ...
        $day = DateCalc::daysInMonth($month,$year);
    ...
    static function nextWeekday($day="",$month="",$year="",$format="%Y%m%d")
    {
        ...
        $days = DateCalc::dateToDays($day,$month,$year);

        if(DateCalc::dayOfWeek($day,$month,$year) == 5)
            $days += 3;
        elseif(DateCalc::dayOfWeek($day,$month,$year) == 6)
            $days += 2;
        else
            $days += 1;

        return(DateCalc::daysToDate($days,$format));
    ...
    static function prevWeekday($day="",$month="",$year="",$format="%Y%m%d")
    {
        ...
        if(DateCalc::dayOfWeek($day,$month,$year) == 1)
            $days -= 3;
        elseif(DateCalc::dayOfWeek($day,$month,$year) == 0)
            $days -= 2;
        else
            $days -= 1;
    ...
    static function nextDayOfWeek($dow,$day="",$month="",$year="",$format="%Y%m%d",$onOrAfter=false)
    {
        ...
        $days = DateCalc::dateToDays($day,$month,$year);
        $curr_weekday = DateCalc::dayOfWeek($day,$month,$year);
    ...
    {
        return(DateCalc::nextDayOfWeek($dow,$day="",$month="",$year="",$format="%Y%m%d",true));
    ...
    {
        return(DateCalc::prevDayOfWeek($dow,$day="",$month="",$year="",$format="%Y%m%d",true));
    ...
    static function nextDay($day="",$month="",$year="",$format="%Y%m%d")
    {
        ...
        return(DateCalc::daysToDate($days + 1,$format));
    ...
    {
        if(!DateCalc::isValidDate($day1,$month1,$year1))
            return -1;
        if(!DateCalc::isValidDate($day2,$month2,$year2))
            return -1;

        return(abs((DateCalc::dateToDays($day1,$month1,$year1))
                    - (DateCalc::dateToDays($day2,$month2,$year2))));
    ...
    static function weeksInMonth($month="",$year="")
    {
		...
		// starts on monday
		if(DateCalc_BEGIN_WEEKDAY == 1)
		{	if(DateCalc::firstOfMonthWeekday($month,$year) == 0) {
				$first_week_days = 1;
			} else {
				$first_week_days = 7 - (DateCalc::firstOfMonthWeekday($month,$year) - 1);
			}
		// starts on saturday
		} elseif(DateCalc_BEGIN_WEEKDAY == 6) {	
			if(DateCalc::firstOfMonthWeekday($month,$year) == 0) {
				$first_week_days = 6;
			} else {
				$first_week_days = 7 - (DateCalc::firstOfMonthWeekday($month,$year) + 1);
		...
		// starts on sunday
			...
			$first_week_days = 7 - DateCalc::firstOfMonthWeekday($month,$year);
		}
		return ceil(((DateCalc::daysInMonth($month,$year) - $first_week_days) / 7) + 1);
    ...
    static function firstOfMonthWeekday($month="",$year="")
    {
        ...
        return(DateCalc::dayOfWeek("01",$month,$year));
    ...
    static function beginOfMonth($month="",$year="",$format="%Y%m%d")
    {
        ...
        return(DateCalc::dateFormat("01",$month,$year,$format));

    } // end of func beginOfMonth

    /**
     * Find the month day of the beginning of week for given date,
     * using DateCalc_BEGIN_WEEKDAY. (can return weekday of prev month.)
     *
     * @param string year in format CCYY, default current local year
     * @param string month in format MM, default current local month
     * @param string day in format DD, default current local day
    ...
    static function beginOfWeek($day="",$month="",$year="",$format="%Y%m%d")
    {
        ...
        $this_weekday = DateCalc::dayOfWeek($day,$month,$year);

	if(DateCalc_BEGIN_WEEKDAY == 1)
	{
		if($this_weekday == 0)
			$beginOfWeek = DateCalc::dateToDays($day,$month,$year) - 6;
		else
			$beginOfWeek = DateCalc::dateToDays($day,$month,$year)
				- $this_weekday + 1;
	}
	else
        	$beginOfWeek = (DateCalc::dateToDays($day,$month,$year)
       ...
       /*  $beginOfWeek = (DateCalc::dateToDays($day,$month,$year)
            - ($this_weekday - DateCalc_BEGIN_WEEKDAY)); */

        return(DateCalc::daysToDate($beginOfWeek,$format));

    } // end of func beginOfWeek

    /**
     * Find the month day of the end of week for given date,
     * using DateCalc_BEGIN_WEEKDAY. (can return weekday
     * of following month.)
     *
     * @param string year in format CCYY, default current local year
     * @param string month in format MM, default current local month
    ...
    static function endOfWeek($day="",$month="",$year="",$format="%Y%m%d")
    {
        ...
        $last_dayOfWeek = (DateCalc::dateToDays($day,$month,$year)
            + (6 - $this_weekday + DateCalc_BEGIN_WEEKDAY));

        return(DateCalc::daysToDate($last_dayOfWeek,$format));
    ...
    static function beginOfNextWeek($day="",$month="",$year="",$format="%Y%m%d")
    {
        ...
        $date = DateCalc::daysToDate(DateCalc::dateToDays($day+7,$month,$year),"%Y%m%d");

        $next_week_year = substr($date,0,4);
        $next_week_month = substr($date,4,2);
        $next_week_day = substr($date,6,2);

        $this_weekday = DateCalc::dayOfWeek($next_week_day,$next_week_month,$next_week_year);

        $beginOfWeek = (DateCalc::dateToDays($next_week_day,$next_week_month,$next_week_year)
            - ($this_weekday - DateCalc_BEGIN_WEEKDAY));
    ...
    static function getCalendarWeek($day="",$month="",$year="",$format="%Y%m%d")
    {
        ...
        $curr_day = DateCalc::beginOfWeek($day,$month,$year,"%E");

            for($counter=0; $counter <= 6; $counter++)
            {
                $week_array[$counter] = DateCalc::daysToDate($curr_day,$format);
                $curr_day++;
            }
    ...
    {
        if(empty($year)) $year = DateCalc::dateNow("%Y");
        if(empty($month)) $month = DateCalc::dateNow("%m");
		$month_array = array();
		// starts on monday
		if(DateCalc_BEGIN_WEEKDAY == 1) {
			if(DateCalc::firstOfMonthWeekday($month,$year) == 0) {
				$curr_day = DateCalc::dateToDays("01",$month,$year) - 6;
			} else {
				$curr_day = DateCalc::dateToDays("01",$month,$year) - DateCalc::firstOfMonthWeekday($month,$year) + 1;
			}
		// starts on saturday
		} elseif(DateCalc_BEGIN_WEEKDAY == 6) {
			if(DateCalc::firstOfMonthWeekday($month,$year) == 0) {
				$curr_day = DateCalc::dateToDays("01",$month,$year) - 1;
			} else {
				$curr_day = DateCalc::dateToDays("01",$month,$year) - DateCalc::firstOfMonthWeekday($month,$year) - 1;
			}
		// starts on sunday		
		} else {
			$curr_day = (DateCalc::dateToDays("01",$month,$year) - DateCalc::firstOfMonthWeekday($month,$year));
		}		
		// number of days in this month
        $daysInMonth = DateCalc::daysInMonth($month,$year);

        $weeksInMonth = DateCalc::weeksInMonth($month,$year);
        for($row_counter=0; $row_counter < $weeksInMonth; $row_counter++)
        {
            for($column_counter=0; $column_counter <= 6; $column_counter++)
            {
                $month_array[$row_counter][$column_counter] = DateCalc::daysToDate($curr_day,$format);
                $curr_day++;
            }
        }
    ...
    static function getCalendarYear($year="",$format="%Y%m%d")
    {
        ...
        for($curr_month=0; $curr_month <=11; $curr_month++)
            $year_array[$curr_month] = DateCalc::getCalendarMonth(sprintf("%02d",$curr_month+1),$year,$format);
    ...
    static function daysToDate($days,$format="%Y%m%d")
    {
        ...
        return(DateCalc::dateFormat($day,$month,$century.$year,$format));
    ...
    static function NWeekdayOfMonth($occurance,$dayOfWeek,$month,$year,$format="%Y%m%d")    {
        ...
        $DOW1 = DateCalc::dayOfWeek($DOW1day,$month,$year);

        $wdate = ($occurance - 1) * 7 + 1 +
                (7 + $dayOfWeek - $DOW1) % 7;

        if( $wdate > DateCalc::daysInMonth($month,$year))
                return -1;
        else
                return(DateCalc::dateFormat($wdate,$month,$year,$format));
    ...
    {
        if(!DateCalc::isValidDate($day,$month,$year))
        {
            $year = DateCalc::dateNow("%Y");
            $month = DateCalc::dateNow("%m");
            $day = DateCalc::dateNow("%d");
        ...
        for($strpos = 0; $strpos < strlen($format); $strpos++)
        {
            ...
            if($char == "%")
            {
                ...
                {
                    case "a":
                        $output .= DateCalc::getWeekdayAbbrname($day,$month,$year);
                        break;
                    case "A":
                        $output .= DateCalc::getWeekdayFullname($day,$month,$year);
                        break;
                    case "b":
                        $output .= DateCalc::getMonthAbbrname($month);
                        break;
                    case "B":
                        $output .= DateCalc::getMonthFullname($month);
                        break;
                    case "d":
                    ...
                    case "e":
                        ...
                        break;
                    case "E":
                        $output .= DateCalc::dateToDays($day,$month,$year);
                        break;
                    case "j":
                        $output .= DateCalc::julianDate($day,$month,$year);
                        break;
                    case "m":
                    ...
                    case "t":
                        ...
                        break;
                    case "w":
                        $output .= DateCalc::dayOfWeek($day,$month,$year);
                        break;
                    case "U":
                        $output .= DateCalc::weekOfYear($day,$month,$year);
                        break;
                    case "y":
    ...
    static function getMonthFullname($month)
    {
        ...
        $month_names = DateCalc::getMonthNames();
    ...
    static function getMonthAbbrname($month,$length=3)
    {
        ...
        if(empty($month))
            $month = DateCalc::dateNow("%m");
        return substr(DateCalc::getMonthFullname($month), 0, $length);
    ...
    static function getWeekdayFullname($day="",$month="",$year="")
    {
        ...
        $weekday_names = DateCalc::getWeekDays();
        $weekday = DateCalc::dayOfWeek($day,$month,$year);
     ...
     *
     * @return string full month name
     * @see DateCalc::getWeekdayFullname
     */
    ...
    static function getWeekdayAbbrname($day="",$month="",$year="",$length=3)
    {
        ...
        if(empty($day))
            $day = DateCalc::dateNow("%d");
        return substr(DateCalc::getWeekdayFullname($day,$month,$year),0,$length);
    } // end func getWeekdayFullname
